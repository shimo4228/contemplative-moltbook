...
Seed file format (Markdown with optional YAML frontmatter):
    ...
    top_k: 50             # optional, default None (no truncation)
    ---

    # Optional title (ignored)

    Seed text body. The whole body (post-frontmatter) is embedded.
"""
...
def _parse_seed_file(path: Path) -> View:
...
class ViewRegistry:
    ...
    def __init__(self, views_dir: Optional[Path] = None) -> None:

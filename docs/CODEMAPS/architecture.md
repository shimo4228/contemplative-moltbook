...
```
  ...
  ~/.config/moltbook/ (MOLTBOOK_HOME, runtime data)
    ...
    views/*.md         (user-editable seed views, falls back to packaged)
    skills/*.md        (behavior patterns, generated by insight)
    rules/*.md         (universal rules, rules-distill Practice/Rationale)
    snapshots/*/       (pivot snapshots, ADR-0020 — manifest + views + constitution + centroids.npz)
  ...
  |                                                      |
  |  core/  (platform-independent, 21 modules)          |
  |    _io.py  config.py  domain.py  prompts.py         |
  |    llm.py  embeddings.py  episode_embeddings.py     |
  |    episode_log.py  knowledge_store.py  memory.py    |

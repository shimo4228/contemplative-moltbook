# ADR-0032: Stance — Contemplative Agent はランタイムエージェントである
...
本姿勢は、[Agent Attribution Practice (AAP)](https://github.com/shimo4228/agent-attribution-practice) リポジトリに、既存 8 ADR を「**他のホストカテゴリの中で動くランタイムエージェントに対する** アカウンタビリティ分配」として再フレーミングする foundational ADR (例: AAP ADR-0009) として昇格させる候補。AAP の既存テーゼ (Security Boundary Model、One External Adapter Per Agent、Human Approval Gate、prohibition-strength hierarchy、causal traceability) は暗黙にランタイム姿勢とホストカテゴリフレーミングに条件付きであり、両方を AAP に昇格させればその条件が明示され、AAP がどのエージェントコミュニティに適用されるかが正しく scope される。

...
- Most behaviour-modifying commands — `distill`, `distill-identity`, `insight`, `skill-reflect`, `rules-distill`, `amend-constitution` — typically operate in **LLM Workflow** mode. They take defined inputs, apply semantic judgement, and promote results deterministically through the [Human Approval Gate](0012-human-approval-gate.md). The approval gate is the structural reason these placements stay honest: it forces the output of the semantic step to land at a deterministic boundary before any state change.
...
- `skill-stocktake` (and to a lesser extent `dialogue`, `meditate`) sits at the boundary between LLM Workflow and Autonomous Agentic Loop. The input space is exploratory and the judgement is semantic, but the output revises design-phase artifacts (skills, rules, identity) rather than executing externally observable side effects. This is a Phase-crossing observation: the work originates in the operation phase but writes back into the design phase. In-repo anchors for the same pattern are [ADR-0016](0016-insight-narrow-stocktake-broad.md) (insight as narrow generator vs stocktake as broad consolidator) and [ADR-0023](0023-skill-as-memory-loop.md) (skill-as-memory loop with usage log + reflective write).
- The remaining quadrants (Algorithmic Search; pure Autonomous Agentic Loop) are not currently routes this project takes. This is a usage observation, not a value judgement on those quadrants.

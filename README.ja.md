...
**エピソードログとナレッジより上の全レイヤーはオプション。** エージェントは `distill`（エピソード → ナレッジ抽出）だけで動作する。アイデンティティだけ、スキル + ルールのみ（憲法なし）、憲法のみ（アイデンティティなし）など、ユースケースに合わせて任意の組み合わせで採用できる。各レイヤーは独立して機能し、段階的に追加できる。

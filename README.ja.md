...
**そしてこの全てがコンシューマーハードウェアで動く。** 3層メモリ（エピソード / ナレッジ / アイデンティティ）、embedding + view リトリーバル、cluster ベースのスキル合成、trust と strength 減衰を伴う bitemporal パターン進化、1,160+ のテスト — その全パイプラインが、単一の Apple Silicon Mac（M1 以降、メモリ 16 GB 程度）+ 2 つのオープンウェイトモデル（生成: **qwen3.5:9b** Q4_K_M 量子化 ~5.5 GB / 埋め込み: **nomic-embed-text** ~274 MB, 768次元）で走る。GPU クラスタ不要、クラウド推論不要。

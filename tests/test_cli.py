...
class TestMigrateIdentity:
    """Tests for `migrate-identity` CLI command (ADR-0024 / ADR-0025)."""

    def _make_args(self, *, dry_run: bool = False):
        args = MagicMock()
        args.dry_run = dry_run
        return args

    def _run(self, tmp_path, identity_path, args):
        audit = tmp_path / "logs" / "audit.jsonl"
        history = tmp_path / "logs" / "identity_history.jsonl"
        with patch("contemplative_agent.cli.IDENTITY_PATH", identity_path), \
    ...
    def test_missing_identity_is_noop(self, tmp_path, capsys):
        identity = tmp_path / "identity.md"
        self._run(tmp_path, identity, self._make_args())
        out = capsys.readouterr().out
        assert "no identity file found" in out

    def test_dry_run_preview_no_write(self, tmp_path, capsys):
        identity = tmp_path / "identity.md"
        identity.write_text("old persona\n", encoding="utf-8")
        audit, history = self._run(tmp_path, identity, self._make_args(dry_run=True))
        out = capsys.readouterr().out
        assert "would migrate (dry-run)" in out
        # No write: file unchanged, no backup, no audit, no history
        assert identity.read_text(encoding="utf-8") == "old persona\n"
        assert not identity.with_suffix(".md.bak.pre-adr0024").exists()
        assert not audit.exists()
        assert not history.exists()

    def test_full_migration(self, tmp_path, capsys):
        identity = tmp_path / "identity.md"
        identity.write_text("old persona\n", encoding="utf-8")
        audit, history = self._run(tmp_path, identity, self._make_args())
        out = capsys.readouterr().out
        assert "migrated to block format" in out
        # Backup + migrated file both present
        backup = identity.with_suffix(".md.bak.pre-adr0024")
        assert backup.exists()
        assert backup.read_text(encoding="utf-8") == "old persona\n"
        migrated_text = identity.read_text(encoding="utf-8")
        assert migrated_text.startswith("---\n")
        assert "old persona" in migrated_text
        # Audit + history both appended
        assert audit.exists()
        assert history.exists()
        hist_entry = json.loads(history.read_text(encoding="utf-8").strip())
        assert hist_entry["source"] == "migration"
        assert hist_entry["block"] == "persona_core"
        assert hist_entry["old_hash"] != hist_entry["new_hash"]

    def test_already_migrated_is_noop(self, tmp_path, capsys):
        identity = tmp_path / "identity.md"
        identity.write_text(
            "---\n"
            "blocks:\n"
        ...
        audit, history = self._run(tmp_path, identity, self._make_args())
        out = capsys.readouterr().out
        assert "already in block format" in out
        assert identity.read_text(encoding="utf-8") == original
        # No backup, no audit, no history for a no-op
        assert not identity.with_suffix(".md.bak.pre-adr0024").exists()
        assert not audit.exists()
        assert not history.exists()

    def test_dry_run_on_already_migrated(self, tmp_path, capsys):
        """Dry-run on a block-format file reports no-op, not 'would migrate'."""
        identity = tmp_path / "identity.md"
        identity.write_text(
            "---\n"
            "blocks:\n"
        ...
        self._run(tmp_path, identity, self._make_args(dry_run=True))
        out = capsys.readouterr().out
        assert "already in block format" in out
        assert "would migrate" not in out


class TestDistillIdentityHistoryHook:
    """ADR-0025: _handle_distill_identity appends identity_history.jsonl
    only when a real write to disk happens. Rejected / staged paths MUST NOT
    append so the history stays consistent with on-disk state."""

    def _make_result(self, identity_path: Path):
        from contemplative_agent.core.distill import IdentityResult
    ...
    def _run(self, tmp_path, identity_path, *, args, approved: bool, make_result):
        audit = tmp_path / "logs" / "audit.jsonl"
        history = tmp_path / "logs" / "identity_history.jsonl"
        ...
        with patch("contemplative_agent.cli.IDENTITY_PATH", identity_path), \
             ...
             patch("contemplative_agent.cli.MOLTBOOK_DATA_DIR", tmp_path), \
             patch("contemplative_agent.cli.AUDIT_LOG_PATH", audit), \
             patch("contemplative_agent.cli.SNAPSHOTS_DIR", snapshots), \
             patch("contemplative_agent.cli.KNOWLEDGE_PATH", tmp_path / "knowledge.json"), \
             patch("contemplative_agent.cli._load_view_registry", return_value=MagicMock()), \
             patch("contemplative_agent.cli._take_snapshot", return_value=None), \
             patch("contemplative_agent.core.distill.distill_identity", return_value=result), \
             patch("contemplative_agent.core.memory.KnowledgeStore") as mock_ks_cls, \
             patch("contemplative_agent.cli._approve_write", return_value=approved):
            mock_ks_cls.return_value = MagicMock()
            _handle_distill_identity(args, MagicMock())
        return audit, history

    def _make_args(self, *, stage: bool = False, dry_run: bool = False):
        args = MagicMock()
        args.stage = stage
        args.dry_run = dry_run
        return args

    def test_approved_write_appends_history(self, tmp_path):
        identity = tmp_path / "identity.md"
        identity.write_text("prior\n", encoding="utf-8")
        audit, history = self._run(
            tmp_path, identity,
            args=self._make_args(),
            approved=True,
            make_result=lambda: self._make_result(identity),
        )
        assert history.exists(), "history log should be written on approved distill"
    ...
    def test_rejected_write_does_not_append_history(self, tmp_path):
        identity = tmp_path / "identity.md"
        identity.write_text("prior\n", encoding="utf-8")
        audit, history = self._run(
            tmp_path, identity,
        ...
        assert not history.exists(), "history MUST NOT be written on rejected distill"

    def test_stage_only_does_not_append_history(self, tmp_path):
        """Staging is pre-approval: history is written on adoption, not staging."""
        identity = tmp_path / "identity.md"
        identity.write_text("prior\n", encoding="utf-8")
        staged_dir = tmp_path / ".staged"
        with patch("contemplative_agent.cli.STAGED_DIR", staged_dir):
            audit, history = self._run(
                tmp_path, identity,
                args=self._make_args(stage=True),
                approved=True,  # irrelevant for stage path
                make_result=lambda: self._make_result(identity),
            )
        assert not history.exists(), "staging MUST NOT append history"


class TestAdoptStagedHistoryHook:
    """ADR-0025: when adopt-staged writes an identity.md, append the
    per-block history entry. Non-identity adoptions stay on the
    unchanged branch."""

    def _stage_identity(self, tmp_path, *, new_text: str, target: Path) -> Path:
        staged_dir = tmp_path / ".staged"
        audit = tmp_path / "logs" / "audit.jsonl"
        item = StageItem("identity.md", new_text, target)
        with patch("contemplative_agent.cli.STAGED_DIR", staged_dir), \
             patch("contemplative_agent.cli.MOLTBOOK_DATA_DIR", tmp_path), \
             patch("contemplative_agent.cli.AUDIT_LOG_PATH", audit):
            _stage_results([item], command="distill-identity")
        return staged_dir

    def _run_adopt(self, tmp_path, staged_dir, target, *, yes: bool = True):
        audit = tmp_path / "logs" / "audit.jsonl"
        history = tmp_path / "logs" / "identity_history.jsonl"
        args = MagicMock()
        args.yes = yes
        with patch("contemplative_agent.cli.STAGED_DIR", staged_dir), \
             patch("contemplative_agent.cli.MOLTBOOK_DATA_DIR", tmp_path), \
             patch("contemplative_agent.cli.IDENTITY_PATH", target), \
             patch("contemplative_agent.cli.IDENTITY_HISTORY_PATH", history), \
             patch("contemplative_agent.cli.AUDIT_LOG_PATH", audit):
            _handle_adopt_staged(args, MagicMock())
        return history

    def test_adopt_identity_appends_history(self, tmp_path):
        identity = tmp_path / "identity.md"
        identity.write_text(
            "---\n"
            "blocks:\n"
        ...
        staged = self._stage_identity(tmp_path, new_text=new_text, target=identity)
        history = self._run_adopt(tmp_path, staged, identity)
        assert history.exists(), "adopting an identity.md staged item MUST append history"
        entry = json.loads(history.read_text(encoding="utf-8").strip())
        assert entry["block"] == "persona_core"
        assert entry["source"] == "distill-identity"
        assert entry["old_hash"] != entry["new_hash"]

    def test_adopt_non_identity_does_not_touch_history(self, tmp_path):
        """A skill adoption takes the unchanged branch — history stays empty."""
        skill_target = tmp_path / "skills" / "a.md"
        staged_dir = tmp_path / ".staged"
        audit = tmp_path / "logs" / "audit.jsonl"
        history = tmp_path / "logs" / "identity_history.jsonl"
        item = StageItem("a.md", "# A\n", skill_target)
        with patch("contemplative_agent.cli.STAGED_DIR", staged_dir), \
             patch("contemplative_agent.cli.MOLTBOOK_DATA_DIR", tmp_path), \
             patch("contemplative_agent.cli.AUDIT_LOG_PATH", audit):
            _stage_results([item], command="insight")
        ...
        assert not history.exists(), "non-identity adoption MUST NOT write identity history"

    def test_adopt_identity_rejected_does_not_append(self, tmp_path):
        """Interactive rejection: write never happens, so no history entry."""
        identity = tmp_path / "identity.md"
        identity.write_text(
            "---\n"
            "blocks:\n"
        ...
        args = MagicMock()
        args.yes = False
        with patch("contemplative_agent.cli.STAGED_DIR", staged), \
             patch("contemplative_agent.cli.MOLTBOOK_DATA_DIR", tmp_path), \
             patch("contemplative_agent.cli.IDENTITY_PATH", identity), \
             patch("contemplative_agent.cli.IDENTITY_HISTORY_PATH", history), \
             patch("contemplative_agent.cli.AUDIT_LOG_PATH", audit), \

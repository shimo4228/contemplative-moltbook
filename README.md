...
**Self-Improving Memory** -- Every layer above the immutable episode log is generated by the agent reflecting on its own logs — no external fine-tuning, no labeled training data. Each promotion from logs to patterns, patterns to skills, skills to rules, or skills to identity passes through a [human approval gate](docs/adr/0012-human-approval-gate.md).

- *Embedding + views* — classification is a query, not state; views are editable semantic seeds ([ADR-0019](docs/adr/0019-discrete-categories-to-embedding-views.md)).
- *Pattern evolution* — a new pattern can trigger LLM-driven re-interpretation of topically-related older patterns, with the old row soft-invalidated and a revised row appended ([ADR-0022](docs/adr/0022-memory-evolution-and-hybrid-retrieval.md), *proposed*).
...
- *Forgetting curve* — retrieval-aware Ebbinghaus decay; patterns that never get used lose weight ([ADR-0021](docs/adr/0021-pattern-schema-trust-temporal-forgetting-feedback.md), *proposed*).
...
- *Provenance tracking* — every pattern carries `source_type` and `trust_score`; MINJA-class memory injection attacks become structurally visible rather than invisible ([ADR-0021](docs/adr/0021-pattern-schema-trust-temporal-forgetting-feedback.md), *proposed*).
- *Identity block separation* — `identity.md` is parsed as frontmatter-addressed named blocks (persona_core, current_goals, …); each block update is logged with its own hash ([ADR-0024](docs/adr/0024-identity-block-separation.md) / [ADR-0025](docs/adr/0025-identity-history-and-migrate-cli.md), *proposed*).
...
**Research-Grade Transparency** -- Every decision is traceable. Immutable logs, distilled outputs, and daily reports are [synced publicly](https://github.com/shimo4228/contemplative-agent-data) for reproducibility. Pivot snapshots ([ADR-0020](docs/adr/0020-pivot-snapshots-for-replayability.md), *proposed*) bundle the full state (manifest + views + constitution + centroid embeddings) so that any distillation run can be replayed bit-for-bit.
...
- Zhong, W., Guo, L., Gao, Q., Ye, H., & Wang, Y. (2023). *MemoryBank: Enhancing Large Language Models with Long-Term Memory.* [arXiv:2305.10250](https://arxiv.org/abs/2305.10250) — Ebbinghaus-style decay with access-reinforced strength; informs retrieval-aware forgetting ([ADR-0021](docs/adr/0021-pattern-schema-trust-temporal-forgetting-feedback.md)).
